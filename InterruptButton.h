...
class InterruptButton {
  private:
    ...
    static bool buttonRegistered(gpio_num_t pinNumber);                     // Used to determine if a button is a registered (valid) button
    ...
    // Action fuction is a helper function used to simplify queueing tasks either from ISR or timer
    static void action(InterruptButton  *btn,                               
                       events           event,                                
                       uint8_t          menuLevel, 
                       BaseType_t       *pxHigherPriorityTaskWoken);        // If this argument is supplied, then it is assumed to be called from an ISR

    static void action(InterruptButton  *btn,                               // Helper function to simplify calling actions at specified menulevel
                       events           event,                                
                       uint8_t          menuLevel);

    // These inline functions allow user to default to current menu level if not specified
    inline static void action(InterruptButton* btn, events event, BaseType_t *pxHigherPriorityTaskWoken) { action(btn, event, m_menuLevel, pxHigherPriorityTaskWoken); };
    ...
    static modes          m_mode;
    static uint64_t       m_btnRegister;                                    // Used to determine if button valid (in case it was deleted) before processing it asychrnously
